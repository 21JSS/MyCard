        ...
        tooltip: {
          ...
          callbacks: {
            label: function (context) {
              return (
                context.dataset.label + ": " + context.parsed.y + " envíos"
              );
            },
          },
        },
      ...
      datasets: [
        {
          ...
          borderColor: ["#f59e0b", "#ef4444", "#94a3b8"],
          borderWidth: 2,
          borderRadius: 8,
          barThickness: 60,
        },
...
function updateDashboardData(data) {
  ...
  if (data.entregados !== undefined) {
    const entregadosCard = document.querySelector(
      ".stat-card.green .stat-value",
    );
    if (entregadosCard) entregadosCard.textContent = data.entregados;

    const entregadosMeta = document.querySelector(
      ".stat-card.green .stat-meta",
    );
    if (entregadosMeta) {
      const tasa =
        data.totalEnvios > 0
          ? ((data.entregados / data.totalEnvios) * 100).toFixed(1)
          : 0;
  ...
  if (data.enTransito !== undefined) {
    ...
    if (transitoMeta) {
      const tasa =
        data.totalEnvios > 0
          ? ((data.enTransito / data.totalEnvios) * 100).toFixed(1)
          : 0;
  ...
  if (data.excepcion !== undefined) {
    const excepcionCard = document.querySelector(
      ".stat-card.orange .stat-value",
    );
    if (excepcionCard) excepcionCard.textContent = data.excepcion;

    const excepcionMeta = document.querySelector(
      ".stat-card.orange .stat-meta",
    );
    if (excepcionMeta) {
      const tasa =
        data.totalEnvios > 0
          ? ((data.excepcion / data.totalEnvios) * 100).toFixed(1)
          : 0;
  ...
  if (data.retornos !== undefined) {
    ...
    if (retornosMeta) {
      const tasa =
        data.totalEnvios > 0
          ? ((data.retornos / data.totalEnvios) * 100).toFixed(1)
          : 0;
...
console.log(
  "📦 Total de envíos: 402 | Entregados: 344 | En tránsito: 43 | Excepciones: 9 | Retornos: 6",
);
